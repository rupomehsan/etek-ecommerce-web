    ...
    <ProfileLayout :bread_cumb="bread_cumb">
        <div class="dashboard position-relative">
            <div class="page-title d-flex justify-content-between">
                <h2>Address book</h2>
                <div>
                    <Link href="address/create" class="btn btn-normal"
                        >Add new</Link
                    >
                </div>
            </div>
                    ...
                    <tbody>
                        <tr v-for="item in user_address_info" :key="item.id">
                            ...
                            <td>
                                <div>
                                    <button
                                        title="show contact person"
                                        type="button"
                                        class="btn btn-info btn-sm"
                                        @click="
                                            getContactPersonByAddress(item.id)
                                        "
                                    >
                                        <i class="fa fa-eye"></i>
                                    </button>
                                    <Link
                                        :href="`address/create?id=${item.id}`"
                                        type="button"
                                        title="Edit"
            ...
            <div v-if="contact_person_modal_show" class="contact-person-modal">
                <div class="">
                    <div class="card">
                        <span
                            @click="contact_person_modal_show = false"
                            class="c-pointer position-absolute"
                            style="right: 10px"
                            >x</span
                        >
                        <div class="card-body">
            ...
            <div v-if="address_contact_form_show" class="address-contact-form">
                <span
                    @click="address_contact_form_show = fasle"
                    class="position-absolute border p-1 m-1 c-pointer text-danger fw-bold"
                    style="right: 0px; top: 0px; z-index: 10"
                    >x</span
                >
...
export default {
    ...
    created: async function () {
        ...
        if (
            authStore.auth_info &&
            authStore.auth_info?.user_delivery_address.length
        ) {
    ...
    methods: {
        ...
        getContactPersonByAddress: async function (id) {
            let response = await window.privateAxios(
                "get-contact-person-by-address-id/" + id
            );
        ...
        DeleteUserAddress: async function (id) {
            var data = await window.s_confirm("Are you sure want to delete?");
            if (data) {
                let response = await window.privateAxios(
                    "delete-user-address-by-id/" + id
                );
                if (response.status == "success") {
                    window.s_alert(response.message);
